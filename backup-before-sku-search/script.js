document.addEventListener('DOMContentLoaded', () => {
    ...
    let removedItems = [];
    // The original currency from the current input (SHEIN API, screenshots, etc.)
    ...
    const convertToSAR = (amount, currency) => {
        return amount / (exchangeRates[currency] || 1);
    };

    const convertFromSAR = (amount, currency) => {
        return amount * (exchangeRates[currency] || 1);
    };

    const getSourcePrice = (item, mode) => {
        if (mode === 'sale') return item.sourceSalePrice ?? convertFromSAR(item.salePrice, sourceCurrency);
        return item.sourceOrigPrice ?? convertFromSAR(item.origPrice, sourceCurrency);
    };
    ...
    const renderCart = () => {
        ...
        cartItems.forEach((it, i) => {
            ...
            const sourcePrice = getSourcePrice(it, mode);
                    ...
                    <div class="product-info">
                        ...
                        <div class="price-row">
                            <span class="current-price" style="display: flex; align-items: center; gap: 8px;">
                                ${formatMoney(sourcePrice, sourceCurrency)} 
    ...
    imagesForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            sourceCurrency = document.getElementById('images-currency').value;
            ...
            cartItems = data.items.map(it => {
                ...
                return {
                    ...
                    salePrice: convertToSAR(sourceSalePrice, sourceCurrency),
                    qty: 1,
                    image: selectedFilesBase64[imgIdx] || selectedFilesBase64[0]
                };
            });
    ...
    const buildOrderText = () => {
        ...
        cartItems.forEach((it, i) => {
            ...
            const sourcePrice = getSourcePrice(it, mode);
            const multiplier = priceSAR > marginThresh ? marginHigh : marginLow;
            const markedPrice = priceSAR * multiplier;
            ...
            text += `   ${formatMoney(sourcePrice, sourceCurrency)} → ${formatMoney(dispTarget, targetCur)}\n`;
